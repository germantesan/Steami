...
export default function IndiePage() {
  ...
  useEffect(() => {
    const fetchJuegos = async () => {
      ...
      if (error) console.error("Error cargando indie:", error);
      setLoading(false);
    };
    fetchJuegos();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#0b121e] text-white p-6 md:p-10 font-sans">
      <header className="mb-10">
        <h1 className="text-4xl font-black text-[#ff4b2b] uppercase tracking-tight">CATEGORÍA: INDIE</h1>
        <p className="text-gray-400 mt-2 italic text-sm">"Si por Indie entiendes juegos de coste bien reducido ajeno a las grandes compañías, este es tu lugar."</p>
      </header>

      {loading ? (
        <div className="text-center p-20 animate-pulse font-black text-[#ff4b2b]">CARGANDO INDIE...</div>
      ) : (
