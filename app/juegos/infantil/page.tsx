  ...
  useEffect(() => {
    const fetchJuegos = async () => {
      ...
      const { data, error } = await supabase
        ...
        /* CORRECCIÓN 1: Usamos 'identificación' para ordenar */
        .order('identificación', { ascending: true });
        
      if (data) setJuegos(data);
      if (error) console.error("Error cargando infantil:", error);
      setLoading(false);
    };
    fetchJuegos();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#0b121e] text-white p-6 md:p-10 font-sans">
      <header className="mb-10">
        <h1 className="text-4xl font-black text-[#ff4b2b] uppercase tracking-tight">CATEGORÍA: INFANTIL</h1>
        <p className="text-gray-400 mt-2 italic text-sm">"Pegi 18. Mantener fuera del alcance de los niños."</p>
      </header>

      {loading ? (
        <div className="text-center p-20 animate-pulse font-black text-[#ff4b2b]">CARGANDO INFANTIL...</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-[1400px] mx-auto">
          {juegos.map((juego, index) => {
            ...
            return (
              <Link 
                /* CORRECCIÓN 2: Usamos identificación para que el enlace funcione */
