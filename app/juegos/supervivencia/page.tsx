  ...
  useEffect(() => {
    const fetchJuegos = async () => {
      ...
      const { data, error } = await supabase
        ...
        /* CORRECCIÓN: Usamos identificación para ordenar en la DB */
        .order('identificación', { ascending: true });
        
      if (data) setJuegos(data);
      if (error) console.error("Error cargando supervivencia:", error);
      setLoading(false);
    };
    fetchJuegos();
  }, []);
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-[1400px] mx-auto">
          {juegos.map((juego, index) => {
            ...
            return (
              <Link 
                /* CORRECCIÓN: Enlace con identificación para evitar el /juego/undefined */
